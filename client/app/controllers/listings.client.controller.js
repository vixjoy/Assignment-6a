angular.module('listings').controller('ListingsController', ['$scope', '$location', '$stateParams', '$state', 'Listings',
  function($scope, $location, $stateParams, $state, Listings){
    ...
    $scope.update = function(isValid) {
      ...
      Listings.update(listing._id, listing).then(function(response) {
        //if the object is successfully saved redirect back to the list page
        $state.go('listings.list', { successMessage: 'New listing succesfully updated!' });
      }, function(error) {
        //otherwise display the error
    ...
    $scope.remove = function() {
      ...
      //console.log("\nMY LISTINGS ID: " + $scope.listings);

      Listings.delete($scope.listing._id).then(function(response){
